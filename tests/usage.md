...
make clean && make BOLOS_SDK=$<device>_SDK      # replace <device> with one of [NANOS, NANOX, NANOSP, STAX, FLEX, APEX_P]
exit
```
...
```
    --device <device>           run the test on the specified device [nanos,nanox,nanosp,stax, flex, apex_p, all]. This parameter is mandatory
